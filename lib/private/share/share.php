...
class Share extends \OC\Share\Constants {
	...
	public static function shareItem($itemType, $itemSource, $shareType, $shareWith, $permissions, $itemSourceName = null, \DateTime $expirationDate = null) {
		...
		if ($shareType === self::SHARE_TYPE_USER) {
			...
			$send = false;
			if ($shareId) {
				$send = self::sendRemoteShare($token, $shareWith, $token, $itemSourceName, $shareId, $uidOwner);
			}
	...
	protected static function unshareItem(array $item, $newParent = null) {
		...
		if ((int)$item['share_type'] === \OCP\Share::SHARE_TYPE_REMOTE) {
	 ...
	 * @return bool
	 */
	private static function sendRemoteShare($token, $shareWith, $token, $name, $remote_id, $owner) {

		list($user, $remote) = explode('@', $shareWith, 2);

		if ($user && $remote) {
			$url = $remote . self::BASE_PATH_TO_SHARE_API;
	...
	private static function sendRemoteUnshare($remote, $id, $token) {
		$url = $remote . self::BASE_PATH_TO_SHARE_API . '/' . $id . '/unshare';
